...
function BlockTemplate(template){
    ...
    this.prevHash = template.prev_hash;
    this.extraNonce = 0;
}
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || (result.prev_hash != currentBlockTemplate.prevHash && result.height >= currentBlockTemplate.height)){
...
function processBlockTemplate(template){
    ...
    if (currentBlockTemplate){
        prevHeight = currentBlockTemplate.height;
    }
    ...
    if (prevHeight === template.height){
        log('info', logSystem, 'Blockchain reorganized at same height %s, new jobs sent to miners', [prevHeight]);
    }
}
...
Miner.prototype = {
    ...
    getJob: function(){
        if (this.lastBlockSeq === currentBlockSeq && !this.pendingDifficulty) {
            return {
                blob: '',
                job_id: '',
                target: ''
        ...
        this.lastBlockSeq = currentBlockSeq;
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        ...
        case 'submit':
            ...
            var blockTemplate = currentBlockSeq === job.blockSeq ? currentBlockTemplate : false;

            if (!blockTemplate){
                log('warn', logSystem, 'Expired share at height %s/%s and thread blockSeq %s/%s by %s@%s',
                    [currentBlockTemplate.height, job.height, currentBlockSeq, job.blockSeq, miner.login, miner.ip]);
